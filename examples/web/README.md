...
## Multi-node (docker + collections-pglite)

`docker-compose.yml` boots the full multi-node topology that end-to-end verifies
`@super-line/collections-pglite`: central **Postgres** + **Electric**, **3
nodes**, a **vite** frontend, and the super-line **Control Center**.
...
(threads/messages/mode, via `@mastra/pg`) and super-line's `collection_rows`
(one shared table carrying all four `harness.*` collections, keyed by pk).
Electric streams that table into each node's in-memory **PGlite replica**;
Mastra reads/writes central PG directly. So any node sees the same threads and
streams the same tree.
...
the frontend container sets `VITE_NODE_BASE_PORT`). To see the collections-pglite proof
...
separate broker-less libp2p/mDNS mesh (independent of the Electric data plane)
...
> If the image build fails on `better-sqlite3` (a transitive dep pulled in by
> `@super-line/collections-sqlite`, unused in pglite mode), the base image needs
> build tools — add
...
telemetry: connections, requests, room broadcasts, collection writes). Watch it
live:
...
- **Suspension**: the supervisor may `ask_user` — the composer switches to
  reply mode.
- **Refresh mid-turn**: the tree rides a durable sqlite collections backend; a reload
